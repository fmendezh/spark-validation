...
object DataValidation {
  ...
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf().setAppName("Simple Application").setMaster("spark://fmendez.gbif.org:7077").set("spark.repl.class.uri", interpreter.sparkIMain.classServerUri).set("spark.submit.deployMode", "client").set("spark.files.userClassPathFirst", "true")
    val sc = new SparkContext(conf)
    interpreter.doQuietly(
      interpreter.bind("sc", "org.apache.spark.SparkContext", sc, List("""@transient"""))
    )
    interpreter.sparkIMain.addUrlsToClassPath(new URL("file:/Users/fmendez/dev/git/gbif/gbif-data-validator/validator-core/target/validator-core-0.1-SNAPSHOT.jar"),
      new URL("file:/Users/fmendez/Downloads/spark-1.6.1-bin-hadoop2.6/lib/spark-assembly-1.6.1-hadoop2.6.0.jar"))
   ...
   val methodRef =
    ...
    interpreter.bind("sqlContext","org.apache.spark.sql.SQLContext", sqlContext, List())
